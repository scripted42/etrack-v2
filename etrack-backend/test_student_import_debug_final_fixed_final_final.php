...
try {
    ...
    // Check all traits
    echo "All traits: " . json_encode(class_uses($import)) . "\n";
    
    // Check if WithHeadingRow is in class_uses
    echo "WithHeadingRow in class_uses: " . (in_array('Maatwebsite\Excel\Concerns\WithHeadingRow', class_uses($import)) ? 'Yes' : 'No') . "\n";
    
    // Check if ToModel is in class_uses
    echo "ToModel in class_uses: " . (in_array('Maatwebsite\Excel\Concerns\ToModel', class_uses($import)) ? 'Yes' : 'No') . "\n";
    
    // Check if WithHeadingRow is in class_implements
    echo "WithHeadingRow in class_implements: " . (in_array('Maatwebsite\Excel\Concerns\WithHeadingRow', class_implements($import)) ? 'Yes' : 'No') . "\n";
    
    // Check if ToModel is in class_implements
    echo "ToModel in class_implements: " . (in_array('Maatwebsite\Excel\Concerns\ToModel', class_implements($import)) ? 'Yes' : 'No') . "\n";
